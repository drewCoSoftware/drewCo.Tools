...
namespace drewCo.Tools
{
  ...
  public partial class ReflectionTools
  {
    ...
    {
      string name = GetNestedPropertyName(memberExp);
      return GetNestedPropertyValue(source, name);
    }
    ...
    public static object GetNestedPropertyValue(object source, string name)
    {
      ...
      {
        PropertyInfo info = GetNestedPropertyInfo(source.GetType(), name);
#if NETFX_CORE
    ...
    /// </remarks>
    /// <param name="baseType">The type that the detected type should derive from. [Optional]</param>
    public static Type ResolveType(string typeName, bool ignoreTypeLoadExceptions = true, bool throwOnNull = false, Type baseType = null)
    {
      AppDomain cDomain = AppDomain.CurrentDomain;
      Assembly[] asms = cDomain.GetAssemblies();
      int asmCount = asms.Length;
      ...
      if (throwOnNull)
      {
        throw new InvalidOperationException(string.Format("Could not find a type with name '{0}'!", typeName));
      }
    ...
    /// This will return all of the attributes that match <typeparamref name="TAttr"/> on the given type.
    /// </summary>
    public static IEnumerable<TAttr> GetAttributeOnType<TAttr>(Type targetType, bool inherit)
    {
      Type attrType = typeof(TAttr);
      foreach (var attr in targetType.GetCustomAttributes(attrType, inherit))
      {
        yield return (TAttr)attr;
      }
    }
